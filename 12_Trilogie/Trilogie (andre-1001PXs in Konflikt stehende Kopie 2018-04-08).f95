...
CONTAINS
  ...
  FUNCTION AND (a,b)
    ...
    IF ((a%VAR == b%VAR) .AND. (a%VAR == 1)) THEN
      AND = TRUE
    ELSE IF ((a%VAR == -1) .OR. (b%VAR == -1)) THEN
      AND = FALSE
    ELSE
      AND = MAYBE
    END IF
  ...
  FUNCTION OR (a,b)
    ...
    IF ((a%VAR == 1) .OR. (b%VAR == 1)) THEN
      OR = TRUE
    ELSE IF ((a%VAR + b%VAR == -1) .OR. (a%VAR + b%VAR == 0)) THEN
      OR = MAYBE
    ELSE
      OR = FALSE
    END IF
